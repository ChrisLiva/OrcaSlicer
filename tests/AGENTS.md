...
- `set_deserialize_strict` does not reject an undeclared key: `PrintConfigDef::handle_legacy` blanks any key `print_config_def` lacks before deserialization, so a misspelled or not-yet-declared key is silently dropped (a bad *value* on a declared key does throw). A test proving a key exists reads it back (`config.option(key) != nullptr`) rather than expecting a throw.

...
TEST_CASE("Placement takes measured printable ground only, and settles what it cannot tell apart by the shorter move", "[MiniatureContacts]")
{
    const ModelSupportRisk::Field field =
...
TEST_CASE("Placement weighs a contact over the width it will be printed at and never rewrites its radius", "[MiniatureContacts]")
{
    const ModelSupportRisk::Field field =
        ModelSupportRisk::build(guard_and_blade(3.), 0.42, []() { return false; });
    REQUIRE(field.status == ModelSupportRisk::Field::Status::Complete);
...
TEST_CASE("The frozen witness lattice halves the extrusion width, aligns to scaled coordinates and keeps every clipped cell", "[MiniatureContacts]")
{
...
TEST_CASE("Legal reach halves the smaller active limit and no influence crosses a hole or a notch", "[MiniatureContacts]")
{
